...
The rest of this paper is organised as follows: The next section presents background information on \myupcase{ecdsa}, the Montgomery ladder and the \fl attack followed by a short discussion of related research. % and the validity of the assumption that an attacker has access to the computer's memory.
...
\item Compute $s=k^{-1}(z+rd_B)\bmod n$; if $s=0$ then return to step \ref{rand_element}.
...
The addition law for points on Weirstrass curves is not complete.
