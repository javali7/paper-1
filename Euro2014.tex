...
collected from these probes to reconstruct the nonce. This attack is a threat to the security of any cryptographic protocol using this scalar multiplication method, when the attacker has access to the target computer's memory. In this paper we illustrate the efficiency of the atttack by analysing ECDSA and recovering the secret key using only one signature at very little computational cost (in both time and memory).
